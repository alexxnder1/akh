                ...
                return new Promise(resolve => {
                    connection_1.default.query('select * from users where discordId=? and guildId=?', [discord_id, guild_id], (err, res) => {
                        ...
                        if (res[0] !== undefined) {
                            ...
                            userDb = res[0];
                            console.log(`[DB Cache] Cache created for user ${discord_id}. Make sure you visit cache.ts to update your db properties.`);
                            this.cache.users.push(userDb);
                            resolve(userDb);
                        }
                        else {
                            (0, userAutoRegistration_1.InsertUserIfNotExists)(discord_id);
                            resolve(null);
                        }
                    });
                });
